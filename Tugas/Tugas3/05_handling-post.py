...
def post_comment():
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
            client_socket.connect(('jsonplaceholder.typicode.com', 80))

            request = b'POST /comments HTTP/1.1\r\nHost: jsonplaceholder.typicode.com\r\nContent-Type: application/json\r\nContent-Length: 98\r\nConnection: close\r\n\r\n{"postId": 1, "name": "Test Name", "email": "test@example.com", "body": "This is a test comment."}'
            client_socket.send(request)

            response = client_socket.recv(4096).decode('utf-8')
            status_code = response.split(' ')[1]
            if status_code == '201':
                comment_id = response.split('"id":')[1].split(',')[0]
