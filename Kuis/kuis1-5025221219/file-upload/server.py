...
class Server:
    ...
    def parse_header(self, header_content):
        ...
        headers, content = header_content.split('\r\n\r\n', 1)
        
        header_1, header_2 = headers.split(',', 1)

        _, filename = header_1.split(': ', 1)
        _, filesize = header_2.split(': ', 1)

        
        return filename, int(filesize), content 
    ...
    def receive_file(self, first_chunk, file_path, file_size, sock):
        ...
        with open(file_path, 'wb') as f:
            ...
            if content_length > 0:
                ...
                total_received = len(first_chunk)

                # write first chunk of data to file
                f.write(first_chunk)

            # Receive and save the file
            # while total received data is less than file size
                ...
                for sock in read_ready:
                    ...
                    if sock == self.server_socket:
                        ...
                        if command != "upload":
                            ...
                            # receive data again
                            new_data = sock.recv(BUFFER_SIZE).decode()

                            # parse header, use parse_header method
                            file_name, file_size, content = self.parse_header(new_data)
