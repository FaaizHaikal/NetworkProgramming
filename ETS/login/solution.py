...
def login(username, password):
    ...
    status, data = make_request('POST', '/login', headers, body = payload)
...
def handle_authenticated_requests(access_token):
    headers = {
        "Authorization": access_token, # TODO: Insert Bearer token
        "Content-Type": "application/json",
    }

    # TODO: GET /auth/me
    status_auth, _ = make_request('GET', 'auth/me', headers)
    ...
    if status_auth == 200:
        ...
        status_update, data_update = make_request(
            'PUT', '/todos/2', headers, put_payload
        )
